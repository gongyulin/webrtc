...
std::vector<std::unique_ptr<PacketBuffer::Packet>> PacketBuffer::FindFrames(
  ...
  for (size_t i = 0; i < buffer_.size(); ++i) {
    ...
    if (buffer_[index]->is_last_packet_in_frame()) {
      ...
      bool is_h264_descriptor = (buffer_[start_index]->codec() == kVideoCodecH264) && !is_generic;
      bool is_h265_descriptor = (buffer_[start_index]->codec() == kVideoCodecH265) && !is_generic;
      ...
      while (true) {
        ...
        if (is_h264_descriptor) {
          ...
          if (is_h265_descriptor && !is_h265_keyframe) {
              const auto* h265_header = absl::get_if<RTPVideoHeaderH265>(&buffer_[start_index]->video_header.video_type_header);
      ...
      if (is_h264_descriptor) {
        ...
        if (is_h265_descriptor) {
            ...
            if (!is_h265_keyframe && missing_packets_.upper_bound(start_seq_num) != missing_packets_.begin()) {
                return found_frames;
            }
        }
