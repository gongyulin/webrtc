...
#include "modules/video_coding/h265_sps_pps_tracker.h"
...
H265SpsPpsTracker::H265SpsPpsTracker() = default;
H265SpsPpsTracker::~H265SpsPpsTracker() = default;

H265SpsPpsTracker::PpsInfo::PpsInfo() = default;
H265SpsPpsTracker::PpsInfo::PpsInfo(PpsInfo&& rhs) = default;
H265SpsPpsTracker::PpsInfo& H265SpsPpsTracker::PpsInfo::operator=(
    PpsInfo&& rhs) = default;
H265SpsPpsTracker::PpsInfo::~PpsInfo() = default;

H265SpsPpsTracker::SpsInfo::SpsInfo() = default;
H265SpsPpsTracker::SpsInfo::SpsInfo(SpsInfo&& rhs) = default;
H265SpsPpsTracker::SpsInfo& H265SpsPpsTracker::SpsInfo::operator=(
    SpsInfo&& rhs) = default;
H265SpsPpsTracker::SpsInfo::~SpsInfo() = default;

H265SpsPpsTracker::FixedBitstream H265SpsPpsTracker::CopyAndFixBitstream(
  ...
  auto& h265_header = absl::get<RTPVideoHeaderH265>(video_header->video_type_header);
  ...
  for (size_t i = 0; i < h265_header.nalus_length; ++i) {
    ...
    switch (nalu.type) {
        ...
        if (video_header->is_first_packet_in_frame) {
          ...
          if (sps->second.data && pps->second.data && pps->second.data) {
  ...
  RTC_CHECK(!append_vps_sps_pps ||
            (vps != vps_data_.end() && sps != sps_data_.end() && pps != pps_data_.end()));
  ...
  H265SpsPpsTracker::FixedBitstream fixed;
  ...
  if (append_vps_sps_pps) {
    ...
    // Update codec header to reflect the newly added VPS SPS and PPS.
    ...
    if (h265_header.nalus_length + 2 <= kMaxNalusPerPacket) {
      ...
      RTC_LOG(LS_WARNING) << "Not enough space in H.264 codec header to insert "
                             "VPS/SPS/PPS provided out-of-band.";
    }
  }
...
void H265SpsPpsTracker::InsertSpsPpsNalus(const std::vector<uint8_t>& vps, const std::vector<uint8_t>& sps,
                                          const std::vector<uint8_t>& pps) {
    constexpr size_t kNaluHeaderOffset = 1;
    if (vps.size() < kNaluHeaderOffset) {
        RTC_LOG(LS_WARNING) << "VPS size  " << sps.size() << " is smaller than "
        << kNaluHeaderOffset;
        return;
    }
    if ((vps[0] & 0x1f) != H265::NaluType::kVps) {
    ...
    if ((sps[0] & 0x1f) != H265::NaluType::kSps) {
        RTC_LOG(LS_WARNING) << "SPS Nalu header missing";
        return;
    }
    ...
    if ((pps[0] & 0x1f) != H265::NaluType::kPps) {
        RTC_LOG(LS_WARNING) << "SPS Nalu header missing";
        return;
    }
    absl::optional<H265VpsParser::VpsState> parsed_vps = H265VpsParser::ParseVps(sps.data() + kNaluHeaderOffset, sps.size() - kNaluHeaderOffset);
    absl::optional<H265SpsParser::SpsState> parsed_sps = H265SpsParser::ParseSps(sps.data() + kNaluHeaderOffset, sps.size() - kNaluHeaderOffset);
    absl::optional<H265PpsParser::PpsState> parsed_pps = H265PpsParser::ParsePps(pps.data() + kNaluHeaderOffset, pps.size() - kNaluHeaderOffset, &parsed_sps.value());
